...
public class Server {
    ...
    private void handleRequest(Request request, BufferedOutputStream out) throws IOException {
    ...
    private void handleMessages(Request request, BufferedOutputStream out) throws IOException {
        ...
        String lastValue = lastParam.isEmpty() ? "all" : lastParam.get(0);

        String responseContent;
        try {
            responseContent = String.format(
                    "{\"status\": \"ok\", \"message\": \"Showing last %s messages\", \"params\": %s}",
                    lastValue,
                    request.getQueryParams()
            );
        } catch (NumberFormatException e) {
            responseContent = "{\"status\": \"error\", \"message\": \"Invalid 'last' parameter\"}";
        }

        byte[] content = responseContent.getBytes();
        final var headers = buildHeaders(200, "OK", "application/json", content.length);
        out.write(headers);
        out.write(content);
        out.flush();
    }
